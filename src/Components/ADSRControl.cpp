...
ADSRControl::ADSRControl(const Control &control, MainDelegate &newDelegate)
...
void ADSRControl::syncComponentProperties(void)
{
    setValueRanges();
    ControlComponent::syncComponentProperties();
}

void ADSRControl::onTouchDown(const TouchEvent &touchEvent)
{
    previousScreenX = touchEvent.getScreenX();
}

void ADSRControl::onTouchMove([[maybe_unused]] const TouchEvent &touchEvent)
{
...
void ADSRControl::onPotTouchDown(const PotEvent &potEvent)
{
    showActiveSegment(true);
    ControlComponent::onPotTouchDown(potEvent);
}

void ADSRControl::onPotChange(const PotEvent &potEvent)
{
...
void ADSRControl::onPotTouchUp(const PotEvent &potEvent)
{
    showActiveSegment(false);
    ControlComponent::onPotTouchUp(potEvent);
}

void ADSRControl::onMidiValueChange(const ControlValue &value,
                                    int16_t midiValue,
                                    uint8_t handle)
{
...
void ADSRControl::paint(Graphics &g)
{
...
void ADSRControl::setValueRanges(void)
{
