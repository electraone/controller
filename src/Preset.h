...
class Preset
{
	public:
		...
		void reset (void);
		bool getPresetNameFast (File &file, char *presetName, size_t maxPresetNameLength);
		void getPresetName (File &file, char *presetName, size_t maxPresetNameLength);
		char *getProjectId (void);
  ...
  public:
	...
	private:
		...
		std::vector<Input> parseInputs (File& file, size_t startPosition, size_t endPosition, ControlType controlType);
		...
		std::vector<Value2> parseValues (File& file, size_t startPosition, size_t endPosition, Control* control);
		...
		std::vector<Rule> parseRules (File& file, size_t startPosition, size_t endPosition);
		Device parseDevice (JsonObject jDevice);
		void parsePatches (File & file, size_t startPosition, size_t endPosition, Device *device);
		void parsePatch (File& file, size_t startPosition, size_t endPosition, Device *device);
		void parseResponses (File& file, size_t startPosition, size_t endPosition, Device *device);
		void parsePatch (JsonObject jPatchItem, Device *device);
		std::vector<Message> parsePostPatch (JsonArray jPostPatch, uint8_t deviceId);
		...
		std::vector<uint8_t> parseData (JsonArray jData, int16_t parameterNumber, ElectraMessageType electraMessageType);
...
bool findElement (File &file, const char *elementName, ElementType elementType, size_t endPosition = 0);
