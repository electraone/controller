...
uint16_t getTwosComplementMidiValue (uint8_t bitWidth, int16_t value,
	  int16_t minValue, int16_t maxValue)
{
...
uint16_t getSignBitMidiValue (uint8_t bitWidth, int16_t value, int16_t minValue,
	  int16_t maxValue)
{
	uint16_t midiValue = 0;
...
uint16_t getUnsignedMidiValue (int16_t value, int16_t minValue,
	  int16_t maxValue, uint16_t midiMin, uint16_t midiMax)
{
	// Add 1 to midiMax when parity differs
	uint16_t midiValue = map (value, minValue, maxValue,
		midiMin, midiMax + (((maxValue-minValue) % 2) != ((midiMax - midiMin) % 2)));
...
uint16_t translateValueToMidiValue (SignMode signMode, uint8_t bitWidth,
	  int16_t value, int16_t minValue, int16_t maxValue, uint16_t midiMin,
		uint16_t midiMax)
{
	uint16_t midiValue = 0;
	...
	{
		midiValue = getTwosComplementMidiValue (bitWidth, value, minValue, maxValue);
	}
	...
	{
		midiValue = getUnsignedMidiValue (value, minValue, maxValue, midiMin,
			  midiMax);
	}
...
int16_t getTwosComplementValue (uint8_t bitWidth, uint16_t midiValue,
	  int16_t minValue, int16_t maxValue)
{
	int16_t value = 0;
...
int16_t getSignBitValue (uint8_t bitWidth, uint16_t midiValue,
	  int16_t minValue, int16_t maxValue)
{
	int16_t value = 0;
...
int16_t getUnsignedValue (uint16_t midiValue, uint16_t midiMin,
	  uint16_t midiMax, int16_t minValue, int16_t maxValue)
{
...
int16_t translateMidiValueToValue (SignMode signMode, uint8_t bitWidth,
	  uint16_t midiValue, uint16_t midiMin, uint16_t midiMax, int16_t minValue,
		int16_t maxValue)
{
	int16_t value = 0;
  ...
  /* TODO: for signed values, check that the midi value is within the range
	...
	{
		value = getUnsignedValue (midiValue, midiMin, midiMax, minValue, maxValue);
	}
