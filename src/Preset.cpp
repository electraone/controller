...
Preset::Preset ()  :
	overlays (Overlays (stringPool)), valid (false)
{
	reset ();
}
...
bool Preset::getPresetNameFast (File &file, char *presetName,
 ...
 */
void Preset::getPresetName (File &file, char *presetName, size_t maxPresetNameLength)
{
	...
	*presetName = '\0';
	...
	copyString (presetName, (*name == '\0') ? "NO NAME" : name, maxPresetNameLength);
	logMessage ("Preset::getPresetName: name=%s", name);
}
...
bool Preset::parseRoot (File &file)
{
	...
	logMessage ("Preset::parseRoot: name=%s, version=%d, projectId=%s", name, version, projectId);
	return (true);
}
 ...
 *
 */
Control Preset::parseControl (JsonObject jControl, File &file)
{
	uint16_t id;
	uint8_t pageId;
	const char *name;
	const char *type;
	const char *mode;
	const char *rgb565Colour;
	const char *variant;
	uint8_t controlSetId;
	bool visible;

	id = jControl["id"].as<uint16_t>();
	pageId = jControl["pageId"].as<uint8_t>();
	name = jControl["name"].as<char *>();
	type = jControl["type"].as<char *>();
	mode = jControl["mode"].as<char *>();
	rgb565Colour = jControl["color"].as<char *>();
	controlSetId = jControl["controlSetId"].as<uint8_t>();
	variant = jControl["variant"].as<char *>();

	if (jControl.containsKey ("visible"))
	{
		visible = jControl["visible"].as<bool>();
	}
	else
	{
		visible = true;
	...
	logMessage ("parseControl: id=%d, name=%s, type=%s (%d), mode=%d, colour=%s, controlSetId=%d, variant=%s, visible=%d",
				id, name, type, controlType, controlMode, rgb565Colour, controlSetId, variant, visible);
	...
	return (Control (id, pageId, name, controlType, controlMode, colour, controlSetId, variantType, visible));
}
...
bool Preset::parseControls (File &file)
{
	...
	do
	{
		...
		if (err)
		{
			logMessage ("Preset::parseControls: deserializeJson() failed: %s", err.c_str ());
			return (false);
		}
		...
		if (jControl)
		{
			Control control = parseControl (jControl, file);
			...
			{
				controls[controlId].setBounds (parseBounds (file, controlStartPosition, controlEndPosition));
				controls[controlId].values = parseValues (file, controlStartPosition, controlEndPosition, &controls[controlId]);
				controls[controlId].inputs = parseInputs (file, controlStartPosition, controlEndPosition, control.getType ());
				pages[controls[controlId].getPageId ()].setHasObjects (true);
			}
			else
			{
				logMessage ("Preset::parseControls: control cannot be inserted: controlId=%d", controlId);
			}
		}
		else
		{
...
bool Preset::parseOverlays (File &file)
{
	...
	do
	{
		...
		DeserializationError err = deserializeJson (doc, file, DeserializationOption::NestingLimit (1));

		if (err && (err.code () != DeserializationError::TooDeep) && (err.code () != DeserializationError::InvalidInput))
		{
			logMessage ("Preset::parseOverlays: deserializeJson() failed: %s", err.c_str ());
			return (false);
		}
		...
		if (jOverlay)
		{
			...
			{
				logMessage ("Preset::parseOverlays: parsing of overlay items has failed");
				return (false);
			}
		}
		else
...
bool Preset::parseOverlayItems (File &file, Overlay *overlay)
{
	...
	do
	{
		...
		{
			logMessage ("Preset::parseOverlayItems: deserializeJson() failed: %s", err.c_str ());
			return (false);
		}
		...
		if (item)
		{
			...
			logMessage ("Preset::parseOverlayItems: value=%d, label=%s, bitmap.empty=%d", value, label, bitmapLocation.isEmpty ());

			overlay->addItem ({ value, stringPool.saveItem (bitmapLocation.getAddress (), label) });
 ...
 *
 */
std::vector<Input> Preset::parseInputs (File& file, size_t startPosition, size_t endPosition, ControlType controlType)
{
 ...
 *
 */
Input Preset::parseInput (File& file, size_t startPosition, ControlType controlType)
{
	Input input;
	const size_t capacity = 128;
	StaticJsonDocument<capacity> doc;
	...
	if (err)
	{
		logMessage ("Preset::parseInput: deserializeJson() failed: %s", err.c_str ());
		return (input);
	}
...
Input Preset::parseInput (ControlType controlType, JsonObject jInput)
{
	...
	valueId = jInput["valueId"] | "";
	potId = jInput["potId"].as<uint8_t>();

	valueIndex = translateValueId (controlType, valueId);

	/* TODO: decrease the potId to make it 0 based */
	if (potId > 0)
	{
		potId--;
	}
	...
	//logMessage ("parseInput: handle=%d, potId=%d", handle, potId);
 ...
 *
 */
std::vector<Value2> Preset::parseValues (File& file, size_t startPosition, size_t endPosition, Control* control)
{
...
Value2 Preset::parseValue (File& file, size_t startPosition, Control* control)
{
	...
	if (err)
	{
		logMessage ("Preset::parseValue: deserializeJson() failed: %s", err.c_str ());
		return (value);
	}
...
Value2 Preset::parseValue (Control* control, JsonObject jValue)
{
	...
	overlayId = jValue["overlayId"].as<uint8_t>();
	 ...
	 */
	if ((controlType != ControlType::pad) && (controlType != ControlType::list))
	{
		if (defaultValue > max)
		{
			defaultValue = max;
	...
	logMessage ("parseValue: id=%s, index=%d, defaultValue=%d, min=%d, max=%d, overlayId=%d, formatter=%s, function=%s", valueId, valueIndex, defaultValue, min, max, overlayId, formatter, function);
	...
	/*
	   ...
	   {
	      logMessage ("C program = %d", message.getOnValue ());
	   }
	 */
	return (Value2 (control, valueId, valueIndex, defaultValue, min, max, overlayId, overlay, message, formatter, function));
}
...
Message Preset::parseMessage (JsonObject jMessage)
{
	...
	bool lsbFirst;
	uint8_t bitWidth;
	const char *signModeInput;

	deviceId = jMessage["deviceId"].as<uint8_t>();
	type = jMessage["type"].as<char *>();
	parameterNumber = jMessage["parameterNumber"].as<int16_t>();
	min = jMessage["min"].as<int16_t>();
	max = jMessage["max"].as<int16_t>();
	value = jMessage["value"].as<int16_t>();
	offValue = jMessage["offValue"] | MIDI_VALUE_DO_NOT_SEND;
	onValue = jMessage["onValue"] | MIDI_VALUE_DO_NOT_SEND;
	lsbFirst = jMessage["lsbFirst"].as<bool>();
	signModeInput = jMessage["signMode"].as<char *>();

	ElectraMessageType electraMessageType = translateElectraMessageType (type);
	std::vector<uint8_t> data = parseData (jMessage["data"], parameterNumber, electraMessageType);
	...
	logMessage ("parseMessage: device=%d, msgType=%s (%d), parameterId=%d, min=%d, max=%d, value=%d, offValue=%d, onValue=%d, lsbFirst=%d, signMode=%d, bitWidth=%d", deviceId, type, electraMessageType, parameterNumber, min, max, value, (offValue == MIDI_VALUE_DO_NOT_SEND) ? -1 : offValue, (onValue == MIDI_VALUE_DO_NOT_SEND) ? -1 : onValue, lsbFirst, signMode, bitWidth);

	return (Message (deviceId, electraMessageType, parameterNumber, min, max, value, offValue, onValue, data, lsbFirst, signMode, bitWidth));
}
...
bool Preset::parseGroups (File &file)
{
	...
	do
	{
		...
		if (err && (err.code () != DeserializationError::InvalidInput))
		{
			logMessage ("Preset::parseGroups: deserializeJson() failed: %s", err.c_str ());
			return (false);
		}
...
Group Preset::parseGroup (JsonObject jGroup, uint8_t defaultGroupId)
{
	...
	groupId = jGroup["groupId"] | defaultGroupId;
	pageId = jGroup["pageId"].as<uint8_t>();
	bounds = parseBounds (jGroup["bounds"]);
	name = jGroup["name"].as<char *>();
	rgb565Colour = jGroup["color"].as<char *>();

	Colour colour = ElectraColours::translateColour (rgb565Colour);

	//logMessage ("parseGroup: pageId=%d, bounds=[%d %d %d %d], name=%s, colour=%d", pageId, bounds.x, bounds.y, bounds.width, bounds.height, name, colour);
...
Rule Preset::parseRule (File& file, size_t startPosition)
{
	...
	if (err)
	{
		logMessage ("Preset::parseRule: deserializeJson() failed: %s", err.c_str ());
		return (rule);
	}
 ...
 *
 */
std::vector<Rule> Preset::parseRules (File& file, size_t startPosition, size_t endPosition)
{
...
Device Preset::parseDevice (JsonObject jDevice)
{
	...
	id = jDevice["id"].as<uint8_t>();
	port = jDevice["port"].as<uint8_t>();
	channel = jDevice["channel"].as<uint8_t>();
	name = jDevice["name"].as<char *>();
	rate = jDevice["rate"].as<uint16_t>();
	...
	logMessage ("parseDevice: device: id=%d, port=%d, channel=%d, name=%s, rate=%d",
	  id, port, channel, name, rate);

	Device device (id, port - 1, channel, rate);
 ...
 *
 */
void Preset::parsePatches (File& file, size_t startPosition, size_t endPosition, Device *device)
{
	file.seek (startPosition);
 ...
 * parsePatch
 */
void Preset::parsePatch (File& file, size_t startPosition, size_t endPosition, Device *device)
{
	...
	if (err)
	{
		logMessage ("Preset::parsePatch: deserializeJson() failed: %s", err.c_str ());
		return;
	}
 ...
 *
 */
void Preset::parseResponses (File& file, size_t startPosition, size_t endPosition, Device *device)
{
	file.seek (startPosition);
	...
	if (findElement (file, "\"responses\"", ARRAY, endPosition))
	{
		do
		{
			...
			if (err)
			{
				logMessage ("Preset::parseResponses: deserializeJson() failed: %s", err.c_str ());
				return;
			}
			...
			response.rules = parseRules (file, responseStartPosition, endPosition);
...
bool Preset::parseDevices (File &file)
{
	...
	do
	{
		...
		if (err)
		{
			...
			{
				logMessage ("Preset::parseDevices: deserializeJson() failed: %s", err.c_str ());
				return (false);
			}
		}
		...
		if (jDevice)
		{
			...
			{
				logMessage ("Preset::parseDevices: max number of devices reached");
				break;
			}
		}
		else
...
{
	return (Rectangle ( jBounds[0].as<uint16_t>() + 13, jBounds[1].as<uint16_t>() + 24, jBounds[2].as<uint16_t>(), jBounds[3].as<uint16_t>() ));
}

/** Parse bounds in a file
 *
 */
Rectangle Preset::parseBounds (File& file, size_t startPosition, size_t endPosition)
{
	Rectangle bounds;
	...
	if (findElement (file, "\"bounds\"", ELEMENT, endPosition))
	{
		...
		if (err)
		{
			logMessage ("Preset::parseBounds: deserializeJson() of bounds failed: %s", err.c_str ());
			return (bounds);
		}
	}
	return (bounds);
 ...
 *
 */
std::vector<uint8_t> Preset::parseData (JsonArray jData, int16_t parameterNumber, ElectraMessageType electraMessageType)
{
				...
				if (strcmp (type, "value") == 0)
				{
					...
					else
					{
						for (JsonObject jRule : jRules)
						{
							...
							uint16_t parameterId = jRule["parameterNumber"].as<uint16_t>();
							uint8_t pPos = jRule["parameterBitPosition"].as<uint8_t>();
							uint8_t bPos = jRule["byteBitPosition"].as<uint8_t>();
...
bool Preset::parsePages (File &file)
{
	...
	do
	{
		...
		if (err)
		{
			logMessage ("Preset::parsePages: deserializeJson() failed: %s", err.c_str ());
			return (false);
		}
...
Rule Preset::parseRule (JsonObject jRule)
{
	...
	if ((bPos + size) > 7)
	{
		logMessage ("parseRule: position of bits in the byte exceeds 7 bits: byteBitPosition=%d, bitWidth=%d", bPos, size);
	}

	if ((pPos + size) > 14)
	{
		logMessage ("parseRule: position of bits in the parameter exceeds 14 bits: parameterBitPosition=%d, bitWidth=%d", pPos, size);
	}
...
{
	std::vector<uint8_t> request = parseData (jRequest, deviceId, ElectraMessageType::sysex);
...
{
	device->postPatch = parsePostPatch (jPatchItem["post"], device->getId ());
	device->requests.push_back (parseRequest (jPatchItem["request"], device->getId ()));
}

/** Parse Post patch messages
 *
 */
std::vector<Message> Preset::parsePostPatch (JsonArray jPostPatch, uint8_t deviceId)
{
...
uint8_t Preset::translateValueId (ControlType type, const char* valueId)
{
	...
	if (type == ControlType::adsr)
	{
		...
		else if ((type == ControlType::adr) && (strcmp (valueId, "release") == 0))
		{
			return (2);
		}
...
bool findElement (File &file, const char *elementName, ElementType elementType, size_t endPosition)
{
	...
	{
		logMessage ("findElement: not present in the range: element=%s, endPosition=%d", elementName, endPosition);
		return (false);
	}
