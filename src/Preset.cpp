...
Control Preset::parseControl(JsonObject jControl, File &file)
{
    ...
    logMessage(
        "parseControl: id=%d, name=%s, type=%s (%d), mode=%d, colour=%s, controlSetId=%d, variant=%s, visible=%d",
        id,
        name,
        type,
        controlType,
...
Device Preset::parseDevice(JsonObject jDevice)
{
    ...
    logMessage(
        "parseDevice: device: id=%d, port=%d, channel=%d, name=%s, rate=%d",
        id,
        port,
        channel,
        name,
    ...
    Device device(id, name, port - 1, channel, rate);
